...
public class Robot extends TimedRobot {
  ...
  private Spark PivotMotor = new Spark(0);
  ...
  long button1_time;
  DoubleSolenoid LaunchShirt = new DoubleSolenoid(4, 0, 1);
  ...
  public void robotInit() {
    ...
    LaunchShirt.set(DoubleSolenoid.Value.kReverse);
  }
  ...
  public void teleopPeriodic() {
    ...
    if (m_Stick.getRawButton(1)) {
      if (LaunchShirtValue == DoubleSolenoid.Value.kForward) {
        ...
        LaunchShirt.set(DoubleSolenoid.Value.kForward);
      }
      button1_time = System.currentTimeMillis();
    } else {
      if (System.currentTimeMillis() - button1_time > 250) {
        LaunchShirtValue = LaunchShirt.get();
      }
    }
    ...
    if (Math.abs(PivotDown) > 0 && Math.abs(PivotUp) > 0) {
      ...
      if (Math.abs(PivotDown) > 0) {
        PivotMotor.set(PivotDown);
      }
      if (Math.abs(PivotUp) > 0) {
        PivotMotor.set(PivotUp);
      }
